...
def kernel_projection(vec):
    ...
    out = []
    out.append(vec)
    # out.append(np.abs(vec)**2)
    #out.append(vec * np.sqrt(2))
    out.append(vec[0::2] * (vec[1::2]))
    # out.append(vec[np.arange(0,2,ks)] * (vec[2::2]))
    return np.concatenate(out, axis=-1)
def nlsg(dataR, calib, kh = 5, kw = 5, lamda = 1e-8, inMat = None, inMatrix = None):
...
def patches(mat, kh, kw):
    [h, w, coil] = mat.shape
    kSize = kh * kw * coil
    kSize = np.size(kernel_projection(np.zeros([kSize,1])))
    ...
    for y in range (h):
        ...
        for x in range(w):
            ...
            inMatrix[num,:] =  kernel_projection(mat[ys][:,xs][:,:,:].reshape(1,-1))
            num = num + 1
    return inMatrix
